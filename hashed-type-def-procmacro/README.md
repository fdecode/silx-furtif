...
Definition of procedural macros:
* `#[derive(HashedTypeDef)]` : procedural macro for deriving `HashedTypeDef`
* `add_hashed_type_def` : procedural macro for adding implementation of `HashedTypeDef` to existing type
* `add_hashed_type_def_param` : procedural macro for adding implementation of `HashedTypeDef<REF>` to existing type
* `build_hasher` : procedural macro for defining common hasher functions  
